...
class TopQueriesAggregator extends AbstractAggregator
{
    ...
    /**
     * @var float duration of fastest request
     */
    protected static $min_duration = null;
    ...
    public function queryDurationIsMoreThenExistsInTopQueriesStack(DatabaseQuery $query)
    {
        if (! empty($this->stack)) {
            ...
            if($query->getDuration() > $this->getMinDuration()){
                return true;
            }
    ...
    public function aggregate(DatabaseQuery $query, $and_save = true)
    {
        ...
        // Перезаписываем время выполнения самого быстрого запроса в стеке
        $last_in_stack = end($this->stack);
        $this->setMinDuration($last_in_stack->getDuration());
    ...
    /**
     * Get cached duration of a fastest query in storage
     *
     * @return float
     */
    public function getMinDuration()
    {
        if(is_null(self::$min_duration)){
            self::$min_duration = $this->getStorageRepository()->get($this->getStorageKeyFastestQueryDuration());
        }

        return isset(self::$min_duration) ? self::$min_duration : 0;
    }

    /**
     * Set duration of a fastest query in storage
     *
     * @param float $duration
     */
    public function setMinDuration($duration)
    {
        self::$min_duration = $duration;
        $this->getStorageRepository()->forever($this->getStorageKeyFastestQueryDuration(), $duration);
    }

    /**
     * {@inheritdoc}
     */
    protected function getStorageKeyFastestQueryDuration()
    {
        return 'database_queries_fastest_query';
    }
}
